...
class RedisClient {
	...
	execute(commands) {
		...
		if (typeof func == "function") {
			return func.apply(this._redis_client, commands)
	...
	attachEvent() {
		...
		this._redis_client.on('ready', () => {
			if (this.mode != "UNIXSOCKET") {
				rl.setPrompt(util.format("%s:%d> ", this._host, this._port));
			} else {
				rl.setPrompt(util.format("%s> ", this._host));
			}
			rl.prompt();
